                            ...
                            <div class="col-12 form-group text-center mb-1 row justify-content-center p-0 m-0"
                                ...
                                <div class="col-xl-6 row justify-content-center" style="border-radius:10px 10px 0 0 ">
                                    <div class="col-xl-4 col-5  no-gutters p-0 text-center">
                                        <label class=" col-form-label " style="font-size:14px;font-weight:bold">Fecha fin dieta:{{$dieta->fecha_fin}}</label>
                                    </div>
                                            <div class=" col-xl-5 col-6 no-gutters p-0 text-center ">
                                                <label class="col-form-label"
                                                    style="font-size:14px">{{ $dieta->created_at }}
                                                </label>
                                            </div>
                                </div>
                            </div>
                            ...
                            <div class="col-12 ">
                                <div class="row justify-content-center align-items-center px-2 mt-xl-2">
                                    <div class="col-9 col-xl-12 row pr-0 pl-4 pl-xl-2 justify-content-center">
                                        <div class="form-group  col-xl-2 row text-xl-center text-left mb-1 ">
                                            <label class="col-xl-12 col-6 col-form-label p-1"><strong>Nombre dieta:</strong></label>
                                            <label
                                        ...
                                        <div class="form-group col-xl-2 row text-xl-center text-left mb-1  ">
                                            <label class="col-xl-12 col-6 col-form-label p-1"><strong>Tipo
                                                    ...
                                                    <label class="col-xl-12 col-5 col-form-label p-1 mb-1">Tipo Gestacional</label>
                                                    @else
                                                    <label class="col-xl-12 col-5 col-form-label p-1 mb-1">Tipo {{ number_format($dieta->tipo_diabetes, 0) }}</label>
                                                    @endif
                                        </div>
                                                {{-- <div class="form-group col-xl-2 row text-xl-center text-left mb-1  ">
                                                ...
                                                <div class="form-group col-xl-4 row text-xl-center text-left mb-1  ">
                                                    <label class="col-xl-12 col-6 col-form-label p-1 text-xl-center"><strong>Observaciones:</strong></label>
                                                    <label
    ...
    <div class="col-md-12 grid-margin stretch-card">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <a onclick="guardarDieta();" class="btn btn-outline-success">Guardar cambios</a>
                <a onclick="guardarNuevaDieta()" class="btn btn-outline-info">Guardar nueva dieta</a>
                                                    ...
                                                    <div>
                                                        <div>
                                                            <div class="container_buscador">
                                                                ...
                                                                <label class="col-12 text-left"><button class="ml-3 py-0 px-2" disabled  title="No se puede ingresar dos veces el mismo alimento" style="border-radius:10px; border:1px solid grey"><i class="fas fa-info"></i></button>
                                                                </label>
                                                                <div class="container_buscador_desayuno container">
                                                                    <div class="row justify-content-center">
                                                                        <div class=" col-8 row w-100">
                                                                       <button class="" disabled style="background-color:white;border-radius:5px">
...
var alimentosLunes = [], alimentosMartes = [], alimentosMiercoles = [], alimentosJueves = [], alimentosViernes = [], alimentosSabado = []; alimentosDomingo = [];
...
var fila=0;
   ...
   function guardarNuevaDieta()
   {
    var semana = {};

semana = {
    alimentosLunes,alimentosMartes,alimentosMiercoles,alimentosJueves,alimentosViernes,alimentosSabado,alimentosDomingo
}
...
$.ajax({
            ...
            let alimentos = [];
                ...
                for(let i =0; i<long ; i++)
                {
                    if(dia[i].horario===nombreComidas[id] && alimento.id === dia[i].id)
                    {
                        swal({
                        title: "Atención",
                        text: "El alimento "+alimento.nombre+" ya se encuentra agregado en "+nombreComidas[id],
                        icon: "info",
                        // buttons: [
                        //     'No, cancelar',
                        //     'Si, aceptar'
    ...
    function iterarCalculoCantidad(dia, id, c1,c2,c3,c4,c5,c6)
    {
        ...
        let cant = Object.keys(dia);
          ...
          let desayunos = [], colacion1 = [], almuerzos = [], colacion2 = [], meriendas = [], cenas = []
            let alimentos = [], desayuno2=[];
            ...
            {
                let totalCb = 0, totalGrasa = 0, totalPro = 0, totalKcal = 0;
           ...
           {
                    let totalCb1 = 0, totalGrasa = 0, totalPro = 0, totalKcal = 0;
                for(let a = 0 ; a < arregloCantidades.length; a++)
                {
        ...
        if(id>=0 && id<6){ alimentosLunes = diaFalso; console.log('content:',alimentosLunes)}
        if(id>=6 && id<12){ alimentosMartes = diaFalso; console.log('content:',alimentosMartes)}
        if(id>=12 && id<18){ alimentosMiercoles = diaFalso; console.log('content:',alimentosMiercoles)}
        if(id>=18 && id<24){ alimentosJueves = diaFalso; console.log('content:',alimentosJueves)}
        if(id>=24 && id<30){ alimentosViernes = diaFalso; console.log('content:',alimentosViernes)}
        if(id>=30 && id<36){ alimentosSabado = diaFalso; console.log('content:',alimentosSabado)}
        if(id>=36 && id<42){ alimentosDomingo = diaFalso; console.log('content:',alimentosDomingo)}
    ...
    function eliminarAlimento(id,fila,alimento_id) 
    {
        ...
        if(id >=0 && id<6)
        {
         ...
         {
                if(id === c &&  alimentosLunes[j]?.id === alimento_id && alimentosLunes[j]?.horario === nombreComidas[c])
                {
                    ...
                    let carb = alimentosLunes[j].carbohidrato, grasa = alimentosLunes[j].grasa, proteina = alimentosLunes[j].proteina;
      ...
      //  CARGA DE ALIMENTOS DEL DIA MARTES
        if(id >=6 && id<12)
        {
            ...
            for(let j =0 ; j < long ; j++)//contenido de martes, tiene los alimentos
            {
                ...
                if(contAlimentoRepetido)
                {
                if(id === c &&  alimentosMartes[j]?.id === alimento_id && alimentosMartes[j]?.horario === nombreComidas[c-6])
                {
                    ...
                    let carb = alimentosMartes[j].carbohidrato, grasa = alimentosMartes[j].grasa, proteina = alimentosMartes[j].proteina;
         ...
         if(id >=12 && id<18)
        {
         ...
         if(contAlimentoRepetido)
         {
                if(id === c &&  alimentosMiercoles[j]?.id === alimento_id && alimentosMiercoles[j]?.horario === nombreComidas[c-12])
                {
                    ...
                    let carb = alimentosMiercoles[j].carbohidrato, grasa = alimentosMiercoles[j].grasa, proteina = alimentosMiercoles[j].proteina;
         ...
         if(id >=18 && id<24)
        {
         ...
         if(contAlimentoRepetido)
         {
                if(id === c &&  alimentosJueves[j]?.id === alimento_id && alimentosJueves[j]?.horario === nombreComidas[c-18])
                {
                    ...
                    let carb = alimentosJueves[j].carbohidrato, grasa = alimentosJueves[j].grasa, proteina = alimentosJueves[j].proteina;
         ...
         if(id >=24 && id<30)
        {
            ...
            for(let j =0 ; j < long ; j++)//contenido de martes, tiene los alimentos
            {
                for(let c = 24 ; c < 30 ; c++) 
                {
                    if(contAlimentoRepetido)
                    {
                if(id === c &&  alimentosViernes[j]?.id === alimento_id && alimentosViernes[j]?.horario === nombreComidas[c-24])
                {
                    ...
                    let carb = alimentosViernes[j].carbohidrato, grasa = alimentosViernes[j].grasa, proteina = alimentosViernes[j].proteina;
           ...
           if(id >=30 && id<36)
        {
            ...
            for(let j =0 ; j < long ; j++)//contenido de martes, tiene los alimentos
            {
                ...
                {
         if(contAlimentoRepetido){
                if(id === c &&  alimentosSabado[j]?.id === alimento_id && alimentosSabado[j]?.horario === nombreComidas[c-30])
                {
                    ...
                    let carb = alimentosSabado[j].carbohidrato, grasa = alimentosSabado[j].grasa, proteina = alimentosSabado[j].proteina;
            ...
            if(id >=36 && id<42)
        {
            ...
            for(let j =0 ; j < long ; j++)//contenido de martes, tiene los alimentos
            {
                ...
                if(id === c &&  alimentosDomingo[j]?.id === alimento_id && alimentosDomingo[j]?.horario === nombreComidas[c-36])
                {
                    ...
                    let carb = alimentosDomingo[j].carbohidrato, grasa = alimentosDomingo[j].grasa, proteina = alimentosDomingo[j].proteina;
            ...
            if(alimentoRepetido(alimento,id,alimentosLunes,0))
            {
                for(let i = 0; i<6 ;i++)
                {
                    ...
                    contadorfila++;
                        ...
                        totalCb += Number(document.getElementById('totalCarbohidrato'+i).textContent)+Number(alimento.carbohidrato);
                        totalGrasa += Number(document.getElementById('totalGrasa'+i).textContent)+Number(alimento.grasa);
                        totalPro += Number(document.getElementById('totalProteina'+i).textContent)+Number(alimento.proteina);
                        totalKcal += Number(document.getElementById('totalKcal'+i).textContent)+Number(alimento.valor_calorico);
...
function cargarDias(semana)
{
    ...
    var lunes = semana[0], martes = semana[1], miercoles = semana[2], jueves = semana[3], viernes = semana[4], sabado = semana[5], domingo = semana[6]
...
function sumarResultados()
{
    ...
    {
    let totalCb = 0, totalGrasa = 0, totalProteina = 0, totalKcal = 0;
    let idInicio = 0, idFin = 0;
    if(dia==0){idInicio = 0; idFin = 6;}
    if(dia==1){idInicio = 6; idFin = 12;}
    if(dia==2){idInicio = 12; idFin = 18;}
    if(dia==3){idInicio = 18; idFin = 24;}
    if(dia==4){idInicio = 24; idFin = 30;}
    if(dia==5){idInicio = 30; idFin = 36;}
    if(dia==6){idInicio = 36; idFin = 42;}     
    ...
    if(valorKcal>1500)
    {
        swal({
                        title: "Atención",
                        text: "El total del día "+arrayDias[dia]+" de los alimentos agregados a sobrepasado las 1500 Kcal",
                        icon: "info",
                        // buttons: [
                        //     'No, cancelar',
                        //     'Si, aceptar'
